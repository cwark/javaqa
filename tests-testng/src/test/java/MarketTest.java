...
public class MarketTest extends Base {
    ...
    public void marketTest() {
        Base.driver.get(PAGE_URL);
//ждём закрузки страницы и параметров фильтрации
        wait.until(titleIs(PAGE_TITLE));
        wait.until(visibilityOf(Base.driver.findElement(By.xpath("//div[@class='search-layout']"))));
...
//сравниваем товары
    ...
    private void sortByPrice(Direction d) {
        String bodyData = Base.driver.findElement(By.xpath("//body")).getAttribute("data-reqid-chain");

        WebElement element = Base.driver.findElement(By.xpath("//a[text()='по цене']"));
        click(element);
    ...
    private void filterByMaker(String pro) {
        ...
        WebElement element = Base.driver.findElement(By.xpath(pro));
        click(element);
    ...
    private void addToCompare(String maker) {
        ...
        WebElement mainElement = Base.driver.findElement(By.xpath(xpath));
        ...
        mainElement = Base.driver.findElement(locator);

        Assert.assertEquals(mainElement.getText(), "Товар " + title + " добавлен к сравнению");

        mainElement = Base.driver.findElement(By.xpath("//div[@class='popup-informer__close image image_name_close']"));
        click(mainElement);
        wait.until(invisibilityOfElementLocated(locator));
    }

    private void goToCompare() {
        WebElement element = Base.driver.findElement(By.xpath("//span[text()='Сравнение']"));
        click(element);
        wait.until(presenceOfElementLocated(By.xpath("//h1[text()='Сравнение товаров']")));
    }

    private void compare() {
        int size = Base.driver.findElements(By.xpath("//div[@class='n-compare-head__image']/img")).size();
        Assert.assertEquals(size, 2);

        WebElement element = Base.driver.findElement(By.xpath("//span[@class='link__inner' and text()='все характеристики']"));
        click(element);

        wait.until(presenceOfElementLocated(By.xpath("//div[@class='n-compare-row-name i-bem' and text()='Операционная система']")));

        element = Base.driver.findElement(By.xpath("//span[@class='link__inner' and text()='различающиеся характеристики']"));
        click(element);
